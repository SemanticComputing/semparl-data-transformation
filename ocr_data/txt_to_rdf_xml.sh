...
do
    ...
    decade="${year:2:1}"
    path=../data/1908-1999/19${decade}0s/fixed_ocr-text/$year
    files=$path/*.txt
...
# Moving files   
    ...
    echo "> Removing redundant individual raw csvs"
    rm $path/*_RAW.csv
...
###################################################
    ...
    then
        ...
        path=../data/1908-1999/19${decade}0s/fixed_ocr-text/$year/toiset
        files=$path/*.txt
...
#############################################################
    ...
    then 
        path=../data/1908-1999/19${decade}0s/fixed_ocr-text/$year/valiliite
        files=$path/*.txt
        script='txt_to_csv_10s.py'
...
# All done - Play notification sound
